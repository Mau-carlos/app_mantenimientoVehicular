...
{
    public partial class Form_Perfiles : Form
    {
        ...
        public Form_Perfiles()
        {
            InitializeComponent();
            this.ttMensaje.SetToolTip(this.txtNombre, "Ingrese el Nombre del Perfil");
        }
        ...
        private void Habilitar(bool valor)
        {
            ...
            this.txtSueldo.ReadOnly = !valor;
            this.txtIdPerfil.ReadOnly = !valor;
        }
        ...
        {
            this.dataListado.DataSource = NegocioPerfiles.Mostrar();
        ...
        {
            this.dataListado.DataSource = NegocioPerfiles.BuscarNombre(this.txtBuscar.Text);
            this.OcultarColumnas();
            lblTotal.Text = "Total de Registros: " + Convert.ToString(dataListado.Rows.Count);
        }

        private void Form_Perfiles_Load(object sender, EventArgs e)
        {
            this.Top = 0;
            this.Left = 0;
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = NegocioPerfiles.Eliminar(Convert.ToInt32(Codigo));
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        perfil._Nombre = txtNombre.Text.Trim().ToUpper();
                        perfil._Sueldo = Convert.ToDouble(txtSueldo.Text);
                        perfil._Descripcion = txtDescripcion.Text.Trim();
                        rpta = NegocioPerfiles.Insertar(perfil);

                    }
                    else
                    {
                        perfil._IdPerfil = Convert.ToInt32(this.txtIdPerfil.Text);
                        perfil._Nombre = txtNombre.Text.Trim().ToUpper();
                        perfil._Sueldo = Convert.ToDouble(txtSueldo.Text);
                        perfil._Descripcion = txtDescripcion.Text.Trim();
                        rpta = NegocioPerfiles.Editar(perfil);
                    }
        ...
        {
            if (!this.txtIdPerfil.Text.Equals(""))
            {
                this.IsEditar = true;
                this.Botones();
                this.Habilitar(true);
        ...
        {
            this.txtIdPerfil.Text = Convert.ToString(this.dataListado.CurrentRow.Cells["id"].Value);
        ...
        {
            Form_ReportePerfiles frm = new Form_ReportePerfiles();
            frm.ShowDialog();
        }
